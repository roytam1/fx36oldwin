...
typedef BOOL (WINAPI *INITIALIZECRITICALSECTIONANDSPINCOUNT)(
    CRITICAL_SECTION *lpCriticalSection,
    DWORD dwSpinCount);

static INITIALIZECRITICALSECTIONANDSPINCOUNT sInitializeCriticalSectionAndSpinCount;
...
void _PR_MD_INIT_LOCKS(void)
{
    ...
    sInitializeCriticalSectionAndSpinCount = (INITIALIZECRITICALSECTIONANDSPINCOUNT)
            GetProcAddress(hKernel32, "InitializeCriticalSectionAndSpinCount");
}
...
PRStatus _PR_MD_NEW_LOCK(_MDLock *lock)
{
    ...
    if (sInitializeCriticalSectionEx) {
        ...
        ok = sInitializeCriticalSectionAndSpinCount(cs, LOCK_SPIN_COUNT);
    } else  {
        ok = 1, InitializeCriticalSection(cs);
    }
    if (!ok) {
